...
final class Silencer implements MessengerInterface {
  ...
  public function __construct(
    ...
    // A side effect of the themeDevelopmentMode config action, which applies
    // itself by submitting the development settings form. Nobody saved any
    // settings, so this is just noise. It only ever becomes visible when
    // installing in a language other than English, because that is the only
    // case where a task with a visible page runs between the recipes being
    // applied and `drupal_cms_installer_finished()` clearing status messages.
    // @see \Drupal\drupal_cms_helper\Plugin\ConfigAction\ThemeDevelopmentMode::apply()
    // @see \Drupal\system\Form\DevelopmentSettingsForm::submitForm()
    self::$reject[] = 'The settings have been saved.';
  }
