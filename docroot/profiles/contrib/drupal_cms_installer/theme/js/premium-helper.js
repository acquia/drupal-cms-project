...
(function (Drupal, once) {
  ...
  function updateValidationUI(licenseKeyInput, state) {
    ...
    const thinkingMessage = statusElement.querySelector('.license-dialog__status-message--thinking');
    const validMessage = statusElement.querySelector('.license-dialog__status-message--valid');
    const invalidMessage = statusElement.querySelector('.license-dialog__status-message--invalid');
    ...
    const statusMessages = [thinkingMessage, validMessage, invalidMessage];
    ...
    if (state === 'loading') {
      ...
      thinkingMessage.hidden = false;
      thinkingMessage.removeAttribute('aria-hidden');
    } else if (state === 'valid') {
      statusElement.classList.add('license-dialog__status--valid');
      validMessage.hidden = false;
      validMessage.removeAttribute('aria-hidden');
      ...
      invalidMessage.removeAttribute('aria-hidden');
  ...
  function init(el) {
    ...
    const editLicenseKeyLabel = licenseDialog.querySelector('.license-dialog__button-label--edit-license-key');
    const enterLicenseKeyLabel = licenseDialog.querySelector('.license-dialog__button-label--enter-license-key');
    ...
    licenseDialog.addEventListener('close', () => {
      ...
      if (isPremiumTemplateChecked() && licenseKeyAppearsGood(currentInput) && !isInvalid(currentInput)) {
        ...
        enterLicenseButton.textContent = editLicenseKeyLabel.textContent.trim();
      }
      else {
        licenseKeyIndicator?.setAttribute('hidden', 'true');
        enterLicenseButton.textContent = enterLicenseKeyLabel.textContent.trim();
      }
    });
  }
