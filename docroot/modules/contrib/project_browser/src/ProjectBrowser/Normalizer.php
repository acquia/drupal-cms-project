...
final class Normalizer implements NormalizerInterface {
  ...
  public function __construct(
    ...
    private readonly CsrfTokenPath $csrfTokenPath,
  ) {}
  ...
  private function getActivationInfo(Project $project, string $source_id, ?AccountInterface $account): array {
    ...
    if ($activator instanceof TasksInterface) {
      ...
      $map = function (Link $link): array {
        ...
        return [
          'text' => (string) $text,
          // Finalize CSRF placeholders so task links work when delivered via
          // JSON to the Svelte UI (outside the HTML render pipeline).
          'url' => $this->csrfTokenPath->toString($url),
