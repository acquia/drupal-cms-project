...
class ConfigSchemaTest extends KernelTestBase {
  ...
  /**
   * Tests that the schema alter does not instantiate plugins.
   *
   * The alter works from the plugin definitions, so the schema of a plugin
   * that cannot be instantiated is altered like any other.
   *
   * @see \Drupal\eca\Hook\ConfigSchemaHooks::configSchemaInfoAlter()
   */
  public function testAlterDoesNotInstantiatePlugins(): void {
    // Pin the fixture premise: the action really cannot be instantiated.
    $this->assertNull($this->container->get('eca.service.action')->createInstance('eca_test_float_config_uninstantiable'));

    $definition = $this->container->get('config.typed')->getDefinition('action.configuration.eca_test_float_config_uninstantiable');
    $this->assertSame('eca_integer_or_token', $definition['mapping']['delay']['type']);
  }
